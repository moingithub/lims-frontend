...
To keep the frontend and backend in sync, use these stable module keys (matching the `modules` table `name` / `module_name` field) everywhere in the UI and API:

| Display Label         | Module Key              | Notes (expected id)\* |
| --------------------- | ----------------------- | --------------------- |
| Dashboard             | `dashboard`             | 1                     |
| Cylinder Check-Out    | `cylinder_checkout`     | 11                    |
| Sample Check-In       | `sample_checkin`        | 3                     |
| Work Orders           | `work_orders`           | 4                     |
| Generate Invoice      | `generate_invoice`      | 5                     |
| Invoices              | `invoices`              | 6                     |
| Analysis Pricing      | `analysis_pricing`      | 7                     |
| Cylinder Master       | `cylinder_master`       | 8                     |
| Company Master        | `company_master`        | 9                     |
| Contacts              | `contacts`              | 10                    |
| Company Areas         | `company_areas`         | 11                    |
| Import Machine Report | `import_machine_report` | 12                    |
| Cylinder Inventory    | `cylinder_inventory`    | 13                    |
| Analysis Reports      | `analysis_reports`      | 14                    |
| Pending Work Orders   | `pending_work_orders`   | 15                    |
| Roles                 | `roles`                 | 16                    |
| Users                 | `users`                 | 17                    |
| Modules               | `modules`               | 18                    |
| Role Module           | `role_module`           | 19                    |

\*IDs are provided for reference; the _keys_ should be treated as the primary contract between frontend and backend. The Sidebar now uses these keys via `moduleName`, and `authService.hasModuleAccessByName` maps them to numeric IDs in one place.
